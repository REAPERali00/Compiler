...
extern nag_intg line; /* source code line number - defined in scanner.c */
...
extern nag_void startParser(nag_void);
extern nag_intg startScanner(ReaderPointer sc_buf);

static nag_void printParserError(nag_char *fmt, ...);
static nag_void displayParser(ReaderPointer ptrBuffer);
static nag_long getParserFileSize(nag_char *fname);
static nag_void callGarbageCollector(nag_void);
...
nag_intg mainParser(nag_intg argc, nag_char **argv) {
  ...
  sourceBuffer =
      readerCreate(READER_DEFAULT_SIZE, READER_DEFAULT_INCREMENT, MODE_MULTI);
  ...
  stringLiteralTable =
      readerCreate(READER_DEFAULT_SIZE, READER_DEFAULT_INCREMENT, MODE_ADDIT);
...
nag_void printParserError(nag_char *fmt, ...) {
...
nag_long getParserFileSize(nag_char *fname) {
  FILE *input;
  nag_long flength;
  input = fopen(fname, "r");
  if (input == NULL) {
...
nag_void displayParser(ReaderPointer ptrBuffer) {
...
* This function is always called despite how the program terminates - normally
*or abnormally.
************************************************************
*/

nag_void callGarbageCollector(nag_void) {
